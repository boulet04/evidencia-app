...
export default function Chat() {
  ...
  // IMPORTANT: chez toi la table est "messages" (pas conversation_messages)
  ...
  // IMPORTANT: messages n’a pas user_id chez toi
  ...
  async function isConversationOwnedByUser({ uid, convId }) {
    const { data, error } = await supabase
      .from("conversations")
      .select("id")
      .eq("id", convId)
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      // 1) convId URL si elle appartient à l'user
      if (convIdFromUrl) {
        const ok = await isConversationOwnedByUser({ uid, convId: convIdFromUrl });
        if (ok) {
      ...
      // 2) sinon dernière conv si existante
      if (h && h.length > 0) {
        const chosen = h[0].id;
        const msgs = await fetchMessages({ convId: chosen });
        if (!mounted) return;
  ...
  async function openConversation(convId) {
    ...
    const ok = await isConversationOwnedByUser({ uid: userId, convId });
    if (!ok) {
      setErrorMsg("Conversation non accessible.");
      return;
    }
  ...
  async function newConversation() {
    ...
    if (!convId) {
      setErrorMsg("Impossible de créer une conversation (RLS/policies Supabase).");
      return;
    }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // IMPORTANT : on affiche l’erreur serveur réelle
      if (!resp.ok) {
      ...
      await insertMessage({ convId: conversationId, role: "assistant", content: msg });
