...
// IMPORTANT: le front lit la table `messages` (voir les calls /rest/v1/messages...).
// Si tu avais `conversation_message` auparavant, c'est exactement ce qui faisait que rien ne s'affichait.
const TABLE_MESSAGES = "messages";


// --- Mistral HTTP API helper (no SDK) ---
async function mistralChatComplete({ apiKey, model, messages, temperature = 0.2, max_tokens = 512 }) {
  ...
  if (!resp.ok) {
    ...
    const err = new Error(`Mistral API error ${resp.status}: ${msg}`);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const convId = safeStr(conversationId).trim(); // ✅ maintenant OPTIONNEL (rétrocompat)
    ...
    // 5) La page chat cree toujours une conversation via /api/conversations/init.
    // Sans conversationId, on ne peut ni historiser ni afficher dans l'UI.
    if (!convId) {
    ...
    for (const s of sources) {
      if (s?.type === "url" && safeStr(s?.value).trim()) {
    ...
    // IMPORTANT: la table `messages` n'a pas forcement de colonne `user_id`.
    // On valide deja que la conversation appartient a l'utilisateur plus haut,
    // donc filtrer par conversation_id suffit.
    let hist = [];
    if (convId) {
    ...
    // 8.5) Persister le message utilisateur AVANT l'appel IA (sinon l'UI le "perd" au reload)
    {
      const { error: insUserErr } = await supabaseAdmin
        .from(TABLE_MESSAGES)
        .insert({ conversation_id: convId, role: "user", content: userMsg });
      if (insUserErr) {
        console.error("Insert user message error:", insUserErr);
        return res.status(500).json({ error: "DB insert failed", detail: insUserErr.message });
      }
    }

    // 9) Appel Mistral
    const completion = await mistralChatComplete({
    ...
    // 10) Persister la réponse assistant
    {
      const { error: insAsstErr } = await supabaseAdmin
        .from(TABLE_MESSAGES)
        .insert({ conversation_id: convId, role: "assistant", content: reply });
      if (insAsstErr) {
        console.error("Insert assistant message error:", insAsstErr);
        // On renvoie quand même la réponse pour ne pas bloquer l'utilisateur
        return res.status(200).json({ reply, warning: "Reply generated but could not be saved" });
      }
    }
