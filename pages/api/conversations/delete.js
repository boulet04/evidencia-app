...
export default function ChatPage() {
  ...
  async function loadMessages(conversationId) {
    ...
    const list = (data || []).filter((m) => {
      // On masque les messages techniques (mémoire / logs)
      if (m.role === "system") return false;
      if ((m.content || "").startsWith("MEMORY:")) return false;
      if ((m.content || "").startsWith("MAIL_LOG:")) return false;
      return true;
    });
  ...
  // Une fois userId + agentSlug connus : garantir une conversation existante + greeting
  useEffect(() => {
    if (!userId || !agentSlug) return;
    (async () => {
      setErrorMsg("");
  ...
  async function deleteConversation(convId) {
    ...
    try {
      ...
      // Si on a supprimé la conversation sélectionnée, basculer sur une autre
      if (selectedConvId === convId) {
        if (firstId) {
          setSelectedConvId(firstId);
          await loadMessages(firstId);
        } else {
          // Plus aucune conversation => recréer une nouvelle avec greeting
          setSelectedConvId("");
          setMessages([]);
          await ensureConversationExistsWithGreeting();
        }
...
const styles = {
  ...
  bubbleAgent: { background: "rgba(255,255,255,0.06)" },
