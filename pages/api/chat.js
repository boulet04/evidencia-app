...
function isCsvSource({ name, mime, path }) {
  const n = String(name || "").toLowerCase();
  const p = String(path || "").toLowerCase();
  const m = String(mime || "").toLowerCase();
  return m.includes("text/csv") || n.endsWith(".csv") || p.endsWith(".csv");
}

function isPdfSource({ name, mime, path }) {
  const n = String(name || "").toLowerCase();
  const p = String(path || "").toLowerCase();
  const m = String(mime || "").toLowerCase();
  return m.includes("application/pdf") || n.endsWith(".pdf") || p.endsWith(".pdf");
}

async function downloadBytesFromStorage(supabase, bucket, path) {
  const { data, error } = await supabase.storage.from(bucket).download(path);
  if (error) throw error;
  if (!data) throw new Error("Fichier introuvable");

  // data est un Blob (Node 18 / browser)
  const ab = await data.arrayBuffer();
  return Buffer.from(ab);
}

function cleanLargeText(s, maxChars) {
  const raw = String(s || "");
  const cleaned = raw
    .replace(/\u0000/g, "")
    .replace(/\s+\n/g, "\n")
...
function buildPdfContextBlock({ fileName, extractedText, warning }) {
  const lines = [];
  lines.push(`SOURCE PDF: ${fileName}`);
  if (warning) lines.push(`Note: ${warning}`);
  if (!extractedText || !String(extractedText).trim()) {
    lines.push("Aucun texte exploitable détecté dans ce PDF (peut-être un PDF scanné / image).");
    return safeTruncate(lines.join("\n"), 8000);
  }
  lines.push("Texte extrait (extrait limité):");
  lines.push(String(extractedText));
  return safeTruncate(lines.join("\n"), 20000);
}

async function persistSourceExtraction({ supabase, userId, agentId, context, sourceIndex, patch }) {
  try {
    const nextContext = { ...(context || {}) };
    const srcs = Array.isArray(nextContext.sources) ? [...nextContext.sources] : [];
    if (!srcs[sourceIndex]) return;
    srcs[sourceIndex] = { ...srcs[sourceIndex], ...patch };
    nextContext.sources = srcs;
    ...
    // cache best-effort (ne bloque pas la requête)
  }
}

async function buildSourcesContext({ supabase, userId, agentId, context, userMessage }) {
  const sources = Array.isArray(context?.sources) ? context.sources : [];
  if (sources.length === 0) return "";

  const blocks = [];

  // limite pour éviter d'injecter trop de texte dans le prompt
  const MAX_PDF_CHARS = 60_000;
  const MAX_TOTAL_PDF_BLOCKS = 3;
  let pdfBlocks = 0;
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    // --- PDF (connaissance)
    if (isPdfSource({ name, mime, path })) {
      if (pdfBlocks >= MAX_TOTAL_PDF_BLOCKS) continue;

      let extracted = String(src?.extracted_text || "");
      let warning = "";

      if (!extracted.trim()) {
        try {
          const buf = await downloadBytesFromStorage(supabase, bucket, path);
          const parsed = await pdfParse(buf);
          extracted = cleanLargeText(parsed?.text || "", MAX_PDF_CHARS);
          if (!extracted.trim() || extracted.trim().length < 40) {
            warning = "Texte très faible: ce PDF semble scanné (image).";
          }

          // cache (best-effort) pour ne pas ré-extraire à chaque chat
          await persistSourceExtraction({
            supabase,
            userId,
            agentId,
            context,
            ...
            patch: {
              ...
              extracted_method: "pdf-parse",
              extracted_at: new Date().toISOString(),
              extracted_chars: extracted.length,
            },
          });
        } catch (e) {
          warning = `Impossible d'extraire le texte du PDF (${e?.message || "erreur"}).`;
          extracted = "";
        }
      }
      ...
      blocks.push(
        buildPdfContextBlock({
          fileName: name || path,
          extractedText: extracted ? safeTruncate(extracted, MAX_PDF_CHARS) : "",
          warning,
        })
      );
    }
  ...
  const guardrails = [
    ...
    "- Ne JAMAIS lister tout un document ni fournir des extractions massives.",
    "- Si une info clé manque, poser UNE question de clarification.",
    "- Limiter la réponse à l'essentiel et proposer d'affiner.",
  ].join("\n");
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // F) Charger sources (CSV/PDF) et enrichir le system prompt
