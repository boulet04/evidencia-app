...
/**
 * Pages Router API route: /api/chat
 * IMPORTANT: You MUST NOT have app/api/chat/route.js at the same time (Next.js conflict).
 * Keep this file and delete: app/api/chat/route.js
 */
...
function getBearerToken(req) {
  const h = safeStr(req.headers?.authorization || req.headers?.Authorization);
...
async function extractFileText(sb, bucket, path, mime) {
  ...
  if (m.startsWith("text/") || m.includes("csv") || m.includes("json") || m.includes("markdown")) {
...
async function buildSourcesContext(sb, sources) {
  ...
  for (const s of sources) {
    ...
    // URL can be stored as {type:"url", url:"..."} or {type:"url", value:"..."}
    ...
    // File: {type:"file", path:"...", name:"...", mime:"..."} (or storage_path)
    const p = safeStr(s?.path || s?.storage_path).trim();
    const name = safeStr(s?.name || s?.filename || (p ? p.split("/").pop() : "")).trim();
...
async function callMistral(messages, model) {
  ...
  return safeStr(j?.choices?.[0]?.message?.content || "").trim();
}
...
async function getAgentSystem(sb, userId, agentSlug) {
  ...
  try {
    const cfg = await sb
      ...
      .select("system_prompt,sources,workflows,prompt,model")
      .eq("user_id", userId)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const agentSlug = safeStr(req.body?.agentSlug).trim();
    const conversationId = safeStr(req.body?.conversationId).trim();
    const message = safeStr(req.body?.message).trim();

    if (!agentSlug || !conversationId || !message) {
      return res.status(400).json({ error: "Missing agentSlug, conversationId or message" });
    }
